...
# Leer de la hoja CARGOS (tiene la data correcta)
...
print(f'Cargados {len(afiliados)} afiliados de hoja CARGOS')
print(f'Muestra cargo: {afiliados[0]["cargo"]}')

# Cargar codigos pre-generados
...
wsv = wbv.active
print('\nVerificacion (primeras 5 filas):')
for row in wsv.iter_rows(min_row=2, max_row=6, values_only=True):
    print(f'  {row[0]:2d} | {str(row[1])[:30]:30s} | UO={str(row[3])[:25]:25s} | CARGO={str(row[4])[:30]}')
print(f'\nHecho: {out}')
